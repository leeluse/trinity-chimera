...
class EvolutionWikiMemory:
    ...
    def is_duplicate(self, agent_id: str, fingerprint: str, dedupe_window: int = 120) -> Tuple[bool, Optional[Dict[str, Any]]]:
    ...
    def _best_accepted(self, agent_id: Optional[str]) -> Optional[Dict[str, Any]]:
        ...
        best = max(
            ...
            key=lambda item: (
                ...
                float((item.get("metrics") or {}).get("trinity_score") or 0.0),
    ...
    def _pick_next_mutation(self, failure_counts: Dict[str, int], mutation_counts: Dict[str, int]) -> str:
        if failure_counts:
            dominant_tag = max(failure_counts.items(), key=lambda item: (int(item[1]), item[0]))[0]
    ...
    def build_prompt_context(self, agent_id: Optional[str], constitution: Dict[str, Any]) -> Dict[str, Any]:
        ...
        failure_summary = [
            {"tag": tag, "count": count}
            for tag, count in sorted(failure_counts.items(), key=lambda item: (-int(item[1]), item[0]))[:6]
        ]
        unexplored = [name for name in MUTATION_DIRECTIONS if mutation_counts.get(name, 0) == 0]
        if not unexplored:
            unexplored = [
    ...
    def log_failure_pattern(self, agent_id: str, reason: str, fingerprint: str, metrics: Optional[Dict[str, Any]] = None) -> None:
        metrics = metrics or {}
