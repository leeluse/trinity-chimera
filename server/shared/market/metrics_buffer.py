...
class BufferedMetrics:
    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        """누적된 메트릭 요약 - Trinity Score v2 전체 지표 포함"""
        if not self.ticks:
            return {}
        ...
        mdds     = [t.mdd for t in self.ticks]
        pfs      = [t.profit_factor for t in self.ticks if t.profit_factor is not None]
        wrs      = [t.win_rate for t in self.ticks if t.win_rate is not None]
        trades   = [t.trade_count for t in self.ticks]
        regimes  = [t.market_regime for t in self.ticks if t.market_regime != "unknown"]
