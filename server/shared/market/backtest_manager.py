...
class BacktestManager:
    ...
    def run_backtest(self, strategy: StrategyInterface, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        # In a real scenario, generate_signals would be called in a loop or vectorized
        for i in range(len(data)):
            # [현대화] 새 규격(plural)과 옛 규격(singular) 모두 대응
            current_slice = data.iloc[:i+1]
            try:
                # 1. 최신 규격 시도 (plural)
                if hasattr(strategy, "generate_signals"):
                    sig_obj = strategy.generate_signals(current_slice, getattr(strategy, "params", {}))
                    entry = getattr(sig_obj, "entry", False)
                    exit_sig = getattr(sig_obj, "exit", False)
                    direction = getattr(sig_obj, "direction", "long")
                    
                    if entry and position == 0:
                        signal = 1 if direction == "long" else -1 # Simplified for legacy loop
                    elif exit_sig and position != 0:
                        signal = -1 if position > 0 else 1 # logic to close
                    else:
                        signal = 0
                else:
                    # 2. 구형 규격 시도 (singular)
            ...
            # Trade execution logic (Modified to support directional closing)
            if signal == 1 and position == 0: # Buy
                ...
                position = shares
                balance = 0
            elif signal == -1 and position > 0: # Sell (Close Long)
                cost_price = self.apply_trading_costs(prices[i], -1)
                sale_value = position * cost_price
                trade_pnl = sale_value - entry_cost
                trades.append(trade_pnl)
                balance = sale_value
                position = 0
            # [Note] Short position handling could be expanded here if needed
