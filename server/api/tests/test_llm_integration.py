...
class TestLLMIntegration(unittest.TestCase):
    ...
    @pytest.mark.skipif(not os.getenv('NVIDIA_NIM_API_KEY'), reason="LLM API key not available")
    async def test_actual_llm_api_call_success(self):
        ...
        code = await client.generate_strategy_code(self.evolution_package)
    ...
    async def test_error_handling_with_mock_service(self):
        ...
        with self.assertRaises(Exception) as context:
            await client.generate_strategy_code(self.evolution_package)

        self.assertIn("API call failed", str(context.exception))

    async def test_retry_logic_on_validation_error(self):
        ...
        with mock.patch.object(StrategyLoader, 'validate_code') as mock_validate:
            ...
            client = EvolutionLLMClient(llm_service=mock_service)
            code = await client.generate_strategy_code(self.evolution_package, max_retries=2)
    ...
    async def test_max_retries_exceeded(self):
        ...
        with mock.patch.object(StrategyLoader, 'validate_code') as mock_validate:
            ...
            with self.assertRaises(SecurityError) as context:
                await client.generate_strategy_code(self.evolution_package, max_retries=2)
    ...
    async def test_self_correction_with_error_context(self):
    ...
    async def test_no_llm_service_fallback(self):
...
def evolution_client():
    """Fixture providing an EvolutionLLMClient instance."""
    return EvolutionLLMClient()


@pytest.fixture
def evolution_package():
    """Fixture providing a standard evolution package."""
    return {
        "current_strategy_code": "class mock_strategy:\n    def generate_signal(self, data):\n        return 1",
        "metrics": {
            "trinity_score": 120,
