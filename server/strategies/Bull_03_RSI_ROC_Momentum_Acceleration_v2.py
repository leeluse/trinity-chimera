...
def generate_signal(train_df: pd.DataFrame, test_df: pd.DataFrame) -> pd.Series:
    """
    Bull Strategy 03
    RSI + ROC Momentum Acceleration Long Only
    ...
    for i in range(len(df_ctx)):
        ...
        if in_pos:
            ...
            # Momentum 전략은 01/02보다 타이트하되,
            # 너무 작은 ATR 때문에 즉시 청산되지 않도록 min/max 클리핑
            stop_dist = np.clip(
                atr_now * 2.0,
                entry_price * 0.018,
                entry_price * 0.055
