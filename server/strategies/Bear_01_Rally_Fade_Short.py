...
def generate_signal(train_df: pd.DataFrame, test_df: pd.DataFrame) -> pd.Series:
    ...
    rally_zone = (
        c <= ema50 + atr14 * 0.50   # 상단만: ema50 저항선 근접 필터 (weak_reversal이 하단 담당)
    )
    ...
    enter = (
        ...
        rally_zone.shift(1).fillna(False) &
        rsi_rally.shift(1).fillna(False) &
        weak_reversal &
        lower_close &
        volume_ok
    )
    ...
    # =========================
    profit_take_zone = (
        rsi14 < 32   # 수정: 38 → 32, 연속하는 하락 추세 더 오래 동승
    )

    short_momentum_failure = (
        (rsi14 > 55) |
        (c > ema50) |
        ((plus_di > minus_di) & (adx14 > 18))
    )

    ema20_reclaim = (
        (c > ema20 + atr14 * 0.25) &
        (c.shift(1) > ema20)   # 전봉도 EMA20 위: 2봉 연속 회복 확인
    )
