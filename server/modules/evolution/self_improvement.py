...
from server.api.models.agent import (
...
class SelfImprovementService:
    ...
    async def request_improvement(
        ...
        return {
            ...
            "status": "started",
            "message": "자율 진화 오케스트레이터에 의해 개선 프로세스가 시작되었습니다."
        }

    async def _run_improvement_process(self, improvement_id: str):
        """
        [CORE WORKFLOW] 개선 프로세스 실행 엔진
        진화의 3단계(백테스트 -> LLM 분석/피드백 -> 개선 완료)를 순차적으로 수행합니다.
        """
        ...
        try:
            ...
            # -----------------------------------------------------------------
            # 2단계: LLM 분석 및 개선안 생성 (LLM Analysis & Recommendation)
            # -----------------------------------------------------------------
            ...
            # -----------------------------------------------------------------
            # 3단계: 개선 프로세스 완료 및 결과 반영 (Finalize Improvement)
            # -----------------------------------------------------------------
    ...
    async def _run_backtest(
        ...
        """
        [STAGE 1] 전략 백테스팅 (Strategy Evaluation)
        신규 제안된 또는 현재의 전략 파라미터를 기반으로 성과 지표를 산출합니다.
        """
        # 실제 백테스팅 엔진 사용
        bt_res = await self.engine.run(
            code=strategy_params.get("code", ""), # 전략 코드가 포함되어 있다고 가정
            agent_id=agent_id,
            context={"timeframe": "1h"}
        )

        if not bt_res.get("success"):
            raise Exception(f"Backtest failed: {bt_res.get('error')}")

        metrics = bt_res["metrics"]

        # BacktestResult 모델로 변환
        return BacktestResult(
            ...
            avg_trade_return=metrics["total_return"] / metrics["total_trades"] if metrics["total_trades"] > 0 else 0
        )
    ...
    async def _analyze_with_llm(
        ...
        """
        [STAGE 2] LLM 분석 및 개선 제안 (LLM Analysis & Proposal)
        백테스트 결과를 바탕으로 LLM이 강점/약점을 분석하고 구체적인 개선 파라미터를 제안합니다.
        """
