...
당신은 10년 이상 경력의 수석 퀀트 트레이더입니다. 아래 순서로 전략을 깊이 분석하세요.

<thought>
## 1. 요청 해석
- 원하는 전략 유형: 추세추종 / 평균회귀 / 변동성 돌파 / 거래량 기반 / 하이브리드 중 무엇인가?
- 목표 시장 조건: 강한 추세장 / 횡보장 / 변동성 급등 구간 중 언제 작동하는가?
- 명시적/암묵적 제약 조건은 무엇인가?

## 2. 전략적 엣지 분석
- 이 전략이 수익을 낼 수 있는 근본 이유 (어떤 시장 비효율성을 이용하는가)?
- 핵심 가설: "X 상황에서 Y 조건이면 Z 방향의 움직임이 통계적으로 우위"
- 유사 전략의 알려진 강점과 약점은?

## 3. 설계 방향
- 주 지표 (primary signal): 무엇을 쓸 것이며, 왜 이 지표인가?
- 보조 필터 (confirmation): 오신호 제거를 위해 무엇을 추가할 것인가?
- 레짐 필터 (regime filter): 언제 거래하지 않을 것인가? (ADX, 변동성, 거래량 기준)
- train_df 활용: 어떤 임계값을 학습 데이터에서 동적으로 계산할 것인가?
- 롱/숏 논리 일관성: 추세추종이면 롱=강세 확인(RSI 중립선 상향 등), 숏=약세 확인으로 대칭을 맞춰라. 역추세라면 그 이유를 명시하라.

## 4. 리스크 예상
- 이 전략이 크게 실패할 시장 조건은 무엇인가?
- MDD 발생 시나리오와 설계 단에서 완화 가능한 방법은?
</thought>
...
rsi_upper = rsi_tr.quantile(0.78)   # 학습 구간 상위 22%
rsi_lower = rsi_tr.quantile(0.22)
vol_baseline = train_df['volume'].mean()
```

**② 시장 레짐 필터 (필수 — 모든 상황에서 거래 금지)**
```python
# 변동성 기반 레짐 (ATR)
tr = pd.concat([(high-low),(high-close.shift()).abs(),(low-close.shift()).abs()],axis=1).max(axis=1)
atr = tr.ewm(span=14, adjust=False).mean()
atr_ma = atr.rolling(50).mean()
regime_ok = atr > atr_ma * 0.7   # 변동성 부족 구간 제외

# 또는 ADX 기반 추세 강도
adx = compute_adx(high, low, close)  # 하단 레시피 참조
regime_ok = adx > 20
```

**③ 3-Tier 신호 구조 (필수)**
```python
# Tier 1: 추세 방향 (long_bias / short_bias)
# Tier 2: 진입 타이밍 (entry_long / entry_short)
# Tier 3: 필터 통과 여부 (filter_ok)
...
bb_width = (bb_upper - bb_lower) / bb_mid   # 변동성 레짐 지표
...
kc_upper = kc_mid + 2*atr; kc_lower = kc_mid - 2*atr
...
# Williams %R
will_r = -100*(highest-close)/(highest-lowest).replace(0,1e-9)

# CCI
tp = (high+low+close)/3
cci = (tp - tp.rolling(20).mean()) / (0.015*tp.rolling(20).std().replace(0,1e-9))

# 거래량 비율 / 급등 감지
vol_ratio = vol / vol.rolling(20).mean().replace(0,1e-9)
vol_surge = vol_ratio > 1.5

# Donchian Channel (추세 돌파)
don_upper = high.rolling(20).max()
don_lower = low.rolling(20).min()
```

---
### 품질 체크리스트 (출력 전 자기 검토)
- [ ] train_df 기반 동적 임계값 1개 이상 사용
- [ ] 시장 레짐 필터 포함 (조건 미충족 시 sig=0)
- [ ] Tier 1/2/3 신호 구조 적용
- [ ] 롱/숏 양방향 신호 모두 구현
- [ ] 롱/숏 논리 정합성 확인 (추세추종 전략에서 롱=강세 확인, 숏=약세 확인의 대칭 유지)
- [ ] `shift(-1)` 등 미래 참조 없음 (Look-ahead bias 방지)
- [ ] **신호 발생 빈도 자가 검증** — AND 조건을 너무 많이 쌓으면 신호가 0건이 됨. 조건 3개 이하로 시작하고, 레짐 필터는 ATR 기반(너무 엄격하지 않게). 전체 구간 신호 ≥ 50건 예상될 것
- [ ] ta / talib / scipy 미사용
...
이 전략의 약점과 개선 방향을 <thought> 태그 안에 철저히 분석하세요:
...
## 채굴 품질 기준 (WFO + Monte Carlo 통과 필수)
...
- **시장 레짐 필터** — 단, ATR 기반으로 너무 엄격하지 않게 (atr > atr_ma * 0.6 수준)
- **⚠️ 신호 발생 빈도 최우선** — AND 조건 과다 적층 금지. 조건이 3개를 초과하면 신호가 0건이 될 수 있음. 전체 구간 기준 ≥ 50건 신호가 발생하도록 임계값 설계
- **롱/숏 양방향** — 단방향 전략 금지
...
## 출력 형식
<thought>
[페르소나+씨드 해석 → 독창적 전략 아이디어 도출 → 3-Tier 신호 구조 설계]
</thought>
