...
def load_data(symbol: str, start: datetime, end: datetime, data_dir: Path, interval: str = "1d") -> pd.DataFrame:
    """Load price data from CSV or fetch if not cached."""

    # Try to load from cache
    cache_file = data_dir / f"{symbol.replace('/', '_').replace('-', '_')}_{interval}.csv"
...
def run_backtest(
    ...
    for i in range(strategy.lookback, len(data)):
        ...
        if position is not None:
            active_stop_loss = position.get('stop_loss', stop_loss)
            active_take_profit = position.get('take_profit', take_profit)
            ...
            if active_stop_loss is not None and unrealized_pnl_pct <= -active_stop_loss:
                force_exit = True
            elif active_take_profit is not None and unrealized_pnl_pct >= active_take_profit:
                force_exit = True
        ...
        if signal.entry and position is None:
            if signal.direction == 'long':
                ...
                position = {
                    ...
                    'direction': 'long',
                    'size': position_size,
                    'stop_loss': signal.stop_loss if signal.stop_loss is not None else stop_loss,
                    'take_profit': signal.take_profit if signal.take_profit is not None else take_profit,
                }
            else:  # short
                position_value = cash * max_position_size
...
def main():
    ...
    parser.add_argument('--interval', '-i', default='1d', help='Data interval (e.g., 1d, 1h)')
    ...
    data = load_data(args.symbol, start, end, data_dir, interval=args.interval)
