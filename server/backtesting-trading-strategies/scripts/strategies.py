...
class Signal:
    ...
    stop_loss: Optional[float] = None   # loss threshold in pct as fraction (e.g. 0.03)
    take_profit: Optional[float] = None  # profit threshold in pct as fraction
...
class DonchianBreakoutATR(Strategy):
    """Donchian breakout + ATR risk + consolidation filter.

    Uses one-bar-offset Donchian channels to avoid lookahead bias.
    Enters on breakout only when recent range indicates consolidation.
    """

    name = "donchian_breakout_atr"
    lookback = 30
    ...
    def generate_signals(self, data: pd.DataFrame, params: Dict[str, Any]) -> Signal:
        ...
        range_atr_mult = float(params.get("range_atr_mult", 6.0))
        sl_atr_mult = float(params.get("sl_atr_mult", 1.5))
        tp_atr_mult = float(params.get("tp_atr_mult", 3.0))

        min_bars = max(period + 2, atr_period + 2, range_bars + 2)
        ...
        # One-bar offset Donchian channel.
        ...
        recent_range = high.iloc[-range_bars:].max() - low.iloc[-range_bars:].min()
        consolidation_ok = recent_range < (range_atr_mult * atr)
        if not consolidation_ok:
            return Signal()

        atr_pct = float(atr / close.iloc[-1])
        stop_loss = max(0.001, sl_atr_mult * atr_pct)
        take_profit = max(0.001, tp_atr_mult * atr_pct)
...
STRATEGIES = {
    ...
    "momentum": Momentum(),
    "donchian_breakout_atr": DonchianBreakoutATR(),
}
