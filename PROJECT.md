...
# PROJECT.md — 트리니티 AI 트레이딩 시스템

> LLM 에이전트들이 스스로 전략을 생성하고, 백테스트로 검증하며,
> 결과를 바탕으로 전략을 개선하는 과정을 실시간으로 반복하는 자율 진화 시스템.
...
```
시장 데이터 (OHLCV + Regime) + 이전 백테스트 결과
│ ▼ 적응형 트리거 (Regime Shift, Performance Decay 등)
│ ▼ LLM (C-mode 요약 컨텍스트) → 전략 생성/개선
│ ▼ 백테스트 실행 (IS/OOS 분리) → 트리니티 스코어 계산
│ ▼ 검증 게이트 (OOS 성과 $\ge$ IS의 70% & 스코어 개선)
│ ▼ Supabase 버전 커밋 $\rightarrow$ 전략 업데이트
│ └──────────────── 반복
```
...
## 2. 전략 생성 방식 (LLM)
에이전트는 두 가지 모드로 전략을 설계합니다.

### 모드 1 — 파라미터 조정 (초기 단계)
기존 전략 템플릿 내의 하이퍼파라미터 수치만 변경.

### 모드 2 — 전략 자유 생성 (목표 단계)
지표 선택, 진입/청산 조건, 포지션 사이징 등 전략의 전 과정을 LLM이 직접 코드로 설계.
- **구현 방식**: 동적 Python 코드 생성 및 샌드박스 실행 (B-Mode).
- **보안**: `ast` 기반 정적 분석 및 `multiprocessing` 타임아웃을 통한 안전한 코드 실행.

---

## 3. LLM 입력 구조
- **시장 상황**: 현재 시장 국면(Market Regime, HMM 기반), 변동성, 주요 지표 트렌드.
- **성과 피드백**: 이전 버전의 트리니티 스코어, MDD(최대 낙폭), 승률, 거래 횟수 및 구체적인 손실 구간 로그.
- **제약 조건**: 사용 가능한 라이브러리 목록, 실행 시간 및 메모리 제한 사항.

---

## 4. 트리니티 스코어 (Trinity Score)
점수 산출 공식:
Trinity Score = 수익률(Return) × 0.40 + 샤프 지수(Sharpe) × 25 × 0.35 + (1 + MDD) × 100 × 0.25
...
├── run                # 통합 실행 스크립트 (front, api, slack)
├── front/             # Next.js 14 대시보드 (포트 3000)
│   └── package.json   # 'npm run front' 등 실행 명령어 포함
├── api/               # FastAPI + WebSocket (포트 8000)
│   ├── main.py        # 서버 진입점 및 엔드포인트
│   └── services/      # 자가 개선 핵심 로직 (진화 오케스트레이터 등)
└── ai_trading/        # 트레이딩 엔진 및 AI 로직
    ├── core/          # HMM 국면 분석, 전략 로더/인터페이스, 백테스트 매니저
    ├── rl/            # Gymnasium 환경 및 강화학습(PPO/SAC) 로직
    ├── agents/        # 에이전트 페르소나 및 전략 생성 프롬프트 로직
    ├── battle/        # 에이전트 간 성과 경쟁 및 포트폴리오 관리
    ├── freqai/        # 학습 데이터 및 피처 엔지니어링 관리
    └── tests/         # 보안 샌드박스 및 단위/통합 테스트
```

---

## 7. 실행 및 인프라
- **데이터베이스**: Supabase (PostgreSQL) - 전략 버전, 성과 지표, 진화 이력 저장.
- **스케줄러**: APScheduler - 에이전트별 자율 진화 루프 주기적 관리.
- **실행 방법**: 루트의 `./run` 스크립트를 통해 모든 서비스 동시 실행 가능.

---

## 8. 주요 설계 결정 (ADR)
- **ADR-001**: 전략 생성 방식 — 템플릿에서 자유 생성으로 점진적 확장 (B-Mode).
- **ADR-002**: LLM 입력 데이터 구성 — 백테스트 결과와 실시간 시장 데이터 결합.
- **ADR-003**: 트리니티 스코어를 LLM의 최적화 목표 지표로 설정.
- **ADR-006**: 과적합 방지를 위한 IS/OOS 검증 및 보수적 매매 비용 모델 적용.
- **ADR-007**: Supabase를 통한 경량화된 전략 이력 및 메트릭 저장소 구축.