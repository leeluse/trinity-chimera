              ...
              {params.map((p, idx) => (
                <div key={idx} className="bg-white/5 border border-white/5 rounded-lg py-1.5 px-2.5 hover:bg-white/[0.07] transition-colors flex flex-col gap-1">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {p.trend && p.trend !== 'neutral' && (
                      <div className={`w-3 h-3 flex items-center justify-center rounded-full ${p.trend === 'up' ? 'bg-[#4ade80]/10 text-[#4ade80]' : 'bg-[#f87171]/10 text-[#f87171]'
                        }`}>
                        <svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="4">
                          <path d={p.trend === 'up' ? "M12 19V5m-7 7l7-7 7 7" : "M12 5v14m-7-7l7 7 7-7"} strokeLinecap="round" strokeLinejoin="round" />
                        </svg>
                      </div>
                    )}
                  </div>
