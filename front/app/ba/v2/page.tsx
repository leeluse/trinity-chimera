  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      try {
        ...
        setAgentPerformance(performances.filter(p => p !== null) as AgentPerformance[]);
        ...
        if (activeAgent !== "전체") {
          ...
          // Update agent performance with timeseries data
          setAgentPerformance(prev => {
            const updated = [...prev];
            const agentIndex = updated.findIndex(a => a.id === agentId);
            if (agentIndex !== -1) {
              updated[agentIndex] = {
                ...updated[agentIndex],
                timeseries: {
                  score: timeseriesResults[0] || [],
                  return: timeseriesResults[1] || [],
                  sharpe: timeseriesResults[2] || [],
                  mdd: timeseriesResults[3] || [],
                  win: timeseriesResults[4] || []
                }
              };
            }
            return updated;
