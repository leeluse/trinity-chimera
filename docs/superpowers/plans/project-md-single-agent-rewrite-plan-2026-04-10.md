...
트리거는 루프를 **보조**하는 수단일 뿐, 루프를 **시작**시키는 조건이 아닙니다.
...
```
BacktestRunner (상시 실행, 15~60분 주기)
  → 4개 에이전트 전략 백테스트 자동 실행
  → 결과를 BacktestResultStream에 적재

BacktestResultStream
  → 최신 N개 결과 버퍼링
  → LLMFeedbackClient에 자동 전달

LLMFeedbackClient
  → 컨텍스트 구성: 현재 전략 코드 + 최근 백테스트 결과 + 이전 실패 이유
  → LLM 호출 → 수정된 전략 코드 수신
  → AST 검증 통과 시 즉시 배포
...
Dashboard (Next.js)
  → 에이전트별 실시간 성과 순위 (Trinity Score 기준)
...
4개 에이전트는 독립적으로 백테스트 → 피드백 → 진화 루프를 돌립니다.

### 1.3 Trinity Score v2 (scoring.py 단일 소스)
...
| 문제 | 위치 | 심각도 |
|------|------|--------|
| 백테스트가 상시 실행되지 않음 — 트리거 후 1회성 실행 | backtest_manager.py | 🔴 Critical |
...
| 자율 트리거 비활성 — force_trigger=False이면 진화 미발생 | trigger.py | 🟠 High |
| 대시보드가 성과 경쟁 구조를 보여주지 않음 | front/ | 🟠 High |
| 로그 파일 없음 — stdout만 사용, 진화 이력 추적 불가 | api/main.py | 🟡 Medium |
...
#### T-001: 백테스트 상시 실행 루프
- **파일**: `ai_trading/core/backtest_runner.py` (신규), `api/main.py`
- **작업**:
  - APScheduler로 15~60분마다 4개 에이전트 백테스트 자동 실행
  - 결과를 `BacktestResultStream`에 적재
  - 에이전트 단위로 오류 격리 — 1개 실패해도 루프 계속 진행
- **완료 기준**: 서버 시작 후 사람 개입 없이 백테스트가 주기적으로 실행됨
- **Claude 접근법**: APScheduler job 설계 → 에이전트별 격리 실행 → 결과 적재 순서로 구현

#### T-002: 백테스트 결과 → LLM 자동 피드백
- **파일**: `ai_trading/agents/llm_client.py`, `ai_trading/agents/orchestrator.py`
- **작업**:
  - 결과 적재 시 `LLMFeedbackClient` 자동 호출
  - 컨텍스트: 현재 전략 코드 + 최근 N회 결과 + 이전 실패 이유 + 탐색한 파라미터 범위
  - LLM 응답 수신 → AST 검증 → 즉시 배포
  - 점수 개선 없어도 다음 루프에서 재시도 (완전 자율)
- **완료 기준**: 백테스트 결과 적재 후 자동으로 전략 갱신까지 발생
- **Claude 접근법**: 이벤트 기반 콜백 구조로 설계. 동기/비동기 처리 방식 선택 후 명시.

#### T-003: LLM 실패 시 명시적 오류 처리
- **파일**: `ai_trading/agents/llm_client.py`
- **작업**:
  - mock 반환 제거
  - 실패 시 `LLMUnavailableError` 발생 → 해당 에이전트 건너뜀, 루프 유지
  - Supabase에 `evolution_failed` 상태 기록
- **완료 기준**: LLM 다운 시 silent fail 없이 오류 기록, 루프 중단 없음

#### T-004: Profit Factor + Win Rate 계산
...
- **작업**:
  ...
  - `BacktestResult`에 `profit_factor`, `win_rate` 필드 추가
- **완료 기준**: 모든 백테스트 결과에 PF + WR 포함

#### T-005: Trinity Score v2 단일 소스화
...
- **파일**: `front/`
- **작업**: 한 화면에 3가지 표시
  1. **에이전트별 실시간 성과 순위** — Trinity Score 기준 랭킹
...
- **작업**: JSON Lines 포맷, 일별 파일 자동 생성
  ...
  # 각 이벤트: agent_id, trigger_reason, mode, score_before, score_after, duration_sec
  ```
...
│   └── services/
│       └── supabase_client.py       # evolution_stats — T-011
├── ai_trading/
│   ├── core/
│   │   ├── strategy_interface.py    # BaseStrategy 인터페이스
│   │   ├── strategy_loader.py       # AST 검증 강화 — T-013
│   │   ├── backtest_manager.py      # PF + WR + IS/OOS — T-004, T-008
│   │   ├── backtest_runner.py       # 상시 실행 루프 — T-001 (신규)
│   │   └── scoring.py               # Trinity Score v2 단일 소스 — T-005 (신규)
│   └── agents/
│       ├── constants.py             # agent_id 정의
│       ├── trigger.py               # 보조 트리거
│       ├── llm_client.py            # 피드백 수신 + 모드 분기 — T-002, T-006
│       ├── orchestrator.py          # 루프 조율 + 롤백 — T-012
│       └── prompts/                 # 에이전트별 프롬프트 — T-007 (신규)
│           ├── momentum_hunter.txt
│           ├── mean_reverter.txt
│           ├── macro_trader.txt
│           └── chaos_agent.txt
└── logs/
    └── evolution_YYYYMMDD.log       # 진화 이벤트 로그 — T-010
```
...
- [ ] 서버 시작 후 사람 개입 없이 15~60분마다 백테스트 자동 실행
- [ ] 백테스트 결과가 LLM에 자동 전달되어 전략 수정까지 발생
...
| 3차 | 시스템 취지 재정렬 — 트리거 중심에서 백테스트 상시 실행 + LLM 자동 피드백 루프 중심으로 재설계. T-001(상시 실행), T-002(자동 피드백), T-009(성과 경쟁 대시보드) 신규 추가. `backtest_runner.py`, `scoring.py`, `prompts/` 신규 추가. | Claude |

---

> **이 문서는 살아있는 문서입니다.**
> Claude는 작업 전 반드시 이 문서를 읽고 시작합니다.
> 트리거는 루프를 보조하는 수단이며, 루프를 시작시키는 조건이 아닙니다.